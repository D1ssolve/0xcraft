...
| Build command | No | Repo-specific shell fallback only. Prefer project diagnostics/build checks for normal verification |
| Test command | No | Repo-specific shell fallback only. Prefer one broader project test pass near the end |
...
>
> **Local verification override:** Use repo inspection for assessment, project diagnostics/build checks for compile verification, and one broader project test pass near the end. Only fall back to shell `dotnet restore`, `dotnet build`, or `dotnet test` when the user explicitly requests raw CLI output or structured tooling fails twice with the same infrastructure/tooling error.
...
5. Run project diagnostics/build checks on the current `net9.0` target to establish a baseline and record any pre-existing warnings. If you explicitly need raw CLI/MSBuild output or suspect stale artifacts that diagnostics cannot clarify, use a clean shell build only as fallback.
...
3. Run project diagnostics/build checks after the TFM and package updates. Use the resulting restore/build diagnostics as the primary work queue. If restore-specific issues are not surfaced clearly and fallback is allowed, run `dotnet restore`. Watch for:
...
4. Continue with project diagnostics/build checks until the project or solution compiles cleanly enough to move into Step 3. Use a clean shell build only under the fallback rule above.
...
1. Run project diagnostics/build checks on the migrated project or solution and confirm the compile errors are resolved.
2. Run one broader project test pass appropriate to the touched module or solution scope.
