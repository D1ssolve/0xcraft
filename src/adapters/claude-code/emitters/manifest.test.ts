...
describe("generateClaudeCodeManifest", () => {
  ...
  test("emits displayName whenever packageMetadata.displayName is set", () => {
    const outputRoot = makeTempDir("0xcraft-claude-manifest-display-emit-");
    ...
    expect(result.manifest.displayName).toBe("0xcraft Agents");
  });

  test("omits displayName when packageMetadata.displayName is absent", () => {
    const outputRoot = makeTempDir("0xcraft-claude-manifest-display-absent-");
    ...
    expect(result.manifest.displayName).toBeUndefined();
  });
